(function(root) {
  ...
  View.prototype.renderGameOver = function(score) {
    ...
    this.addButtonLinks();
    this.setTweetLink(score);
  };

  View.prototype.addButtonLinks = function() {
    var submitButton = document.getElementsByClassName("submit-button")[0];
    var that = this;
    submitButton.addEventListener("click", that.showHighScores, false);
  };

  View.prototype.setTweetLink = function(score) {
    var tweetText = score.toString() + " on Head Rush. Flyin' high!";
    var encodedTweetText = encodeURI(tweetText);
    var newLink = "https://twitter.com/intent/tweet?text=" + encodedTweetText;
    var tweetLink = document.getElementsByClassName("tweet-score-link")[0];
    tweetLink.href = newLink;
  };

  View.prototype.showHighScores = function() {
