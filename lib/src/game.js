(function(root) {
  ...
  Game.prototype.handleObstacles = function() {
    if (this.obstacles.length === 0 && this.obstaclesOffView()) {
      if (this.level < 5) {
        this.level += 1;
        this.view.levelUp();
        this.allObjects.push(new Slader.Level(this.view, this.level));
        this.cycleForLevelUp = this.cycles;
      }
      this.obstacles = Slader.Obstacle.generateRandomObstacles(this);
    }

    if (this.cycles - this.cycleForLevelUp < 500) {
      // this.view.renderLevelTransition();
    }
  ...
  Game.prototype.cleanUp = function() {
    for (var i = 0; i < this.allObjects.length; i++) {
      ...
      } else if (this.allObjects[i] instanceof Slader.Level) {
        var lev = this.allObjects[i];
        if (lev.posX + lev.width < 0) {
          this.allObjects.splice(i, 1);
          i -= 1;
        }
      }
